		...
		<table align="center" cellpadding="20px" cellspacing="6px">
			...
			<?php
				...
				if (isset($_POST['ascending'])) {
					$count=0;
					$read_venue = "SELECT * FROM venue_details INNER JOIN event_details ON event_details.UserID='$uid' ORDER BY VenueName ASC";
					...
					$count=0;
					$read_venue = "SELECT * FROM venue_details INNER JOIN event_details ON  event_details.UserID='$uid' ORDER BY VenueName DESC";
					$result_read_venue = mysqli_query($conn, $read_venue);
					if(mysqli_num_rows($result_read_venue)>0){
						while($row = mysqli_fetch_array($result_read_venue, MYSQLI_ASSOC)){
							$count=$count+1;
