...
<body background="..\image\concrete1.jpg">
	...
	<div id="view" align="center">
		...
		<hr>
		<form action="artist_manage_view.php" method="POST" style="font-size: 20px;">
		...
		<table align="center" cellpadding="20px" cellspacing="6px">
			...
			<?php
				if (isset($_POST['ascending'])) {
					...
					$read_user = "SELECT * FROM artist ORDER BY Artist_Name ASC";
					...
					$count=0;
					$conn = mysqli_connect($servername, $username, $password, $dbname);
					$read_user = "SELECT * FROM artist ORDER BY Artist_Name DESC";
					$result_read_user = mysqli_query($conn, $read_user);
					if(mysqli_num_rows($result_read_user)>0){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							$count=$count+1;
					...
					$count=0;
					$conn = mysqli_connect($servername, $username, $password, $dbname);
					$read_user = "SELECT * FROM artist ORDER BY Artist_Id ASC";
					$result_read_user = mysqli_query($conn, $read_user);
					if(mysqli_num_rows($result_read_user)>0){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							$count=$count+1;
