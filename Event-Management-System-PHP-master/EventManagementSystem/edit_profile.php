<?php
	...
	if(isset($_POST['editpicture'])){
		...
		if($newpicture==false){
			...
			$update_picture = "UPDATE guest SET Guest_Image='".$newpicture."' WHERE Guest_Id='$uid'";
			$result_update_picture = $conn->query($update_picture); 
			$update_picture_name = "UPDATE guest SET UserImageName='".$newpicture_name."' WHERE Guest_Id='$uid'";
	...
	if(isset($_POST['editname'])){
		...
		$update_name = "UPDATE guest SET Guest_FName='$newname' WHERE Guest_Id='$uid'";
		$result_update_name = mysqli_query($conn, $update_name);
		if($result_update_name){
			$_SESSION['Guest_FName']=$newname;
		...
		$update_email = "UPDATE guest SET Guest_email='$newemail' WHERE Guest_Id='$uid'";
...
<html>
<head>
	<title>Urban Management - Edit Event</title>
	<style type="text/css">
		a:hover{
			font-size: 24px;
		}
