 <?php
	...
	if (isset($_POST['register'])) {
		...
		$insert_user = "INSERT INTO guest (Guest_Id,	Guest_FName,	Guest_LName,	Guest_email,	Guest_phone,	Guest_CNIC,	Guest_Password) VALUES ('$uid', '$uname', '$ulname', '$uemail', '$uphone','$ucnic','$upass')";
	...
	if (isset($_POST['login'])) {
		...
		echo "$uid $upass";
		$conn = mysqli_connect($servername, $username, $password, $dbname);
		$read_DB = "SELECT * FROM guest WHERE Guest_Id='$uid' AND  Guest_Password='$upass'";
		...
		if(mysqli_num_rows($result)>0){
			while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
				...
				if (strcmp($upass, $row['Guest_Password']) == 0) {
					 
    				session_start();
					$_SESSION['UserFullName'] = $row['Guest_FName'];
...
<html>
<head>
	<title>Urban Management - Login / Register</title>
	<style>
		.login_button{
...
<body background="image\concrete1.jpg" >
	...
	<form method="POST" class="registerform" action="login_register.php">
		...
		<br><br>
		<input type="text" name="First_Name" placeholder="Guest_Fname" required>
		<br><br>
		<input type="text" name="Last_Name" placeholder="Guest_LName" required>
		<br><br>
		...
		<br><br>
		<input type="text" name="Phone_no" placeholder="Guest_phone" required>
		<br><br>
		<input type="text" name="CNIC" placeholder="Guest_CNIC" required>
		<br><br>
		<!-- <br><br>
