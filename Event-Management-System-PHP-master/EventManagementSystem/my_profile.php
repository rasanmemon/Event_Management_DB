		...
		<table cellspacing="1">
			...
			<tr>
				<td rowspan="5" width="35%">
					<?php
						$user_image = "SELECT Guest_Image FROM guest WHERE Guest_Id='$uid'";
						...
						if($result_user_image){
							while($row = mysqli_fetch_array($result_user_image, MYSQLI_ASSOC)){
								echo "<img src='data:image/png;base64,".base64_encode($row['Guest_Image'])."'>";
							}
						}
					?>
				</td>
			...
			<tr>
				<?php
					$read_user = "SELECT Guest_Id FROM guest WHERE Guest_Id='$uid'";
					...
					if($result_read_user){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							...
							echo "<td width='50%'>".$row['Guest_Id']."</td>";
						}
					}
				?>
			</tr>
			<tr>
				<?php
					$read_user = "SELECT Guest_FName FROM guest WHERE Guest_Id='$uid'";
					$result_read_user = mysqli_query($conn, $read_user);
					if($result_read_user){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							echo "<td style='text-align:right;' width='16%'><b>Name: </b></td>";
							echo "<td width='50%'>".$row['Guest_FName']."</td>";
						}
					}
				?>
			</tr>
			<tr>
				<?php
					$read_user = "SELECT Guest_email FROM guest WHERE guest_Id='$uid'";
					$result_read_user = mysqli_query($conn, $read_user);
					if($result_read_user){
						while($row = mysqli_fetch_array($result_read_user, MYSQLI_ASSOC)){
							echo "<td style='text-align:right;' width='16%'><b>E-mail: </b></td>";
							echo "<td width='50%'>".$row['Guest_email']."</td>";
						}
					}
				?>
			</tr>
